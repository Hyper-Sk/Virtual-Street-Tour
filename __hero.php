<section class="hero">
  <div class="background">
    <img src="./images/background/hero-2.jpg" alt="">
  </div>
  <div class="overlay">

  </div>
  <div class="hero-content">
    <h1>Take a Virtual Vacation from Home</h1>
    <!-- <h3>Explore Beautiful Places Around the World</h3> -->
    <p>
      Enjoy a virtual vacation by exploring beautiful beaches, mountains, cities, landmarks, and scenic destinations from home. Experience virtual travel and discover amazing places anytime with immersive virtual tours.
    </p>
    <a class="link" href="./walk.php">Start Your Virtual Journey <i class="uil uil-globe"></i></a>
    <!-- <a class="icon" href="#godown"><i class="bi bi-chevron-compact-down"></i> </a> -->
  </div>
</section>